<template> 
    ...
    <div id='room'>
        ...
        <header>
            ...
            <dl>
                <!-- 个人头像 -->
                <dt @click='testtt,$refs.onidMessageChild.idMessage=true' >
                    <img :src='$store.state.user.userImg' />
                </dt>
                <!-- 房主 -->
                <dd>
                    {{user.type == 1 ? "房总分：": "分数："}}
                    <span>{{user.type == 1 ? Number(user.allFen)+Number(user.oxWater.replace(user.oxWater.slice(-3),"")) : user.myFen}}</span>
                     &nbsp;
        ...
        <div class="weibu">
            ...
            <div class='control'>
                ...
                <span class='control_center' @touchend='move.liType = -1'>
                    ...
                    :class='chouma.one==1?"tuqi":""' @click='main.minFen_x>=1 ? control(1,1):""'
                     :fen='"1"'/>
                     <img :src="`src/srcImg/c${main.minFen_x>=5 ? 'c' : 'h'}02.png`" 
                    :class='chouma.one==2?"tuqi":""' @click='main.minFen_x>=5 ? control(1,2):""'
                     :fen='"5"'/>
                     <img :src="`src/srcImg/c${main.minFen_x>=10 ? 'c' : 'h'}03.png`" 
                    :class='chouma.one==3?"tuqi":""' @click='main.minFen_x>=10 ? control(1,3):""'
                     :fen='"10"'/>
                     <img :src="`src/srcImg/c${main.minFen_x>=50 ? 'c' : 'h'}04.png`" 
                    :class='chouma.one==4?"tuqi":""' @click='main.minFen_x>=50 ? control(1,4):""'
                     :fen='"10"'/>
                     <img :src="`src/srcImg/c${main.minFen_x>=100 ? 'c' : 'h'}05.png`" 
                    :class='chouma.one==5?"tuqi":""' @click='main.minFen_x>=100 ? control(1,5):""'
                     :fen='"10"'/>
                </span>
            ...
            list () {           // 玩家列表
                ...
                function host(){
                    if(self.dataList.z.dian==undefined && self.user.auto && self.user.type== 1){
                        console.log(self.dataList.dd[0].zn_member_id)
                ...
                if(card!=undefined){
                    for(var i=0; i<card.length; i++){
                        if(card[i].few==self.logic.z_index){     // 庄
                            ...
                            if(water != NaN){
                                self.user.oxWater+=water.toFixed(2);
                            }
